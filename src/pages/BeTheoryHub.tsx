...
export const BeTheoryHub = () => {
  ...
  const categories = ['All', 'Hard Skill', 'OOP', 'Database', 'API & Security', 'System Architecture', 'Infrastructure'];
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'Hard Skill': return <Terminal className="w-4 h-4" />;
      case 'OOP': return <Cpu className="w-4 h-4" />;
      case 'Database': return <Database className="w-4 h-4" />;
      case 'API & Security': return <Lock className="w-4 h-4" />;
      case 'System Architecture': return <Workflow className="w-4 h-4" />;
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-background text-foreground overflow-hidden relative">
      ...
      `}>
        ...
        <div className="p-4 border-b space-y-4 bg-background/40">
          <div className="relative group">
            ...
            <Input 
              placeholder="Searching Back-End concepts..." 
              className="pl-10 h-10 border-0 bg-muted/50 focus-visible:ring-1 focus-visible:ring-emerald-500 shadow-none rounded-lg text-xs" 
              value={searchTerm}
            ...
            {categories.map(cat => (
              <Badge 
                ...
                className={`cursor-pointer px-2 py-0.5 flex items-center gap-2 group border-0 shadow-sm transition-all ${
                  activeCategory === cat ? 'bg-emerald-600 scale-105' : 'bg-muted/50 hover:bg-muted'
                }`}
      ...
      <main className="flex-grow bg-background overflow-y-auto h-screen">
          ...
          {selectedTopic ? (
            ...
            >
              <div className="mb-10 space-y-4">
                <Badge className="bg-emerald-600 uppercase tracking-widest text-[9px] py-1 px-3">
                  {selectedTopic.category}
                </Badge>
                <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">
                  {selectedTopic.title}
                </h1>
                <p className="text-lg md:text-xl text-muted-foreground border-l-4 border-emerald-500 pl-6 py-2 italic font-medium">
              ...
              <div className="grid grid-cols-1 gap-12">
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3 border-b pb-4">
                    ...
                    <h3 className="text-xl md:text-2xl font-bold">Logic & Architectural Concept</h3>
                  </div>
                  <div className="prose prose-slate dark:prose-invert max-w-none text-base md:text-lg leading-relaxed text-muted-foreground font-medium">
                ...
                <section className="space-y-6">
                  <div className="flex items-center gap-3 border-b pb-4">
                    ...
                    <h3 className="text-xl md:text-2xl font-bold">Implementation Snippets</h3>
                  </div>
                  <div className="rounded-2xl overflow-hidden shadow-2xl border transition-all duration-300 hover:shadow-emerald-500/10">
                    <SyntaxHighlighter 
                      language="javascript" 
                      style={atomDark}
                ...
                <div className="pt-20 border-t flex flex-col md:flex-row items-center justify-between gap-6 text-muted-foreground text-xs font-mono uppercase tracking-widest">
                   <div className="flex flex-col items-center md:items-start gap-2">
                      <span>© 2026 MOCHRKS BE_EXPERTISE</span>
                      <a href="https://github.com/mochrks" target="_blank" rel="noopener noreferrer" className="hover:text-emerald-500 transition-colors flex items-center gap-2 px-3 py-1 bg-muted rounded-full text-[9px]">
                   ...
                   <Button variant="ghost" className="text-[10px] hover:text-emerald-500" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            ...
            <div className="flex flex-col items-center justify-center h-full text-center p-8">
              <Server className="w-20 h-20 text-muted mb-6" />
              <h2 className="text-xl md:text-2xl font-bold text-muted-foreground uppercase tracking-widest">Select a Backend Topic</h2>
              <p className="text-muted-foreground text-sm">Master your back-end engineering craft from fundamental to distributed systems.</p>
            </div>
          )}
        </AnimatePresence>
      </main>
